...
class KPlaneField(nn.Module):
    def __init__(
        ...
        self.direction_net = tcnn.Network(
            n_input_dims=self.feature_dim + 16,
        ...
        self.color_net_param = torch.load('plenoxels/models/vgg/checkpoints/vgg_mlp_decoder.pth')
        with torch.no_grad():
            self.color_net.params.copy_(self.color_net_param['color_net.params'])
        self.color_net.params.requires_grad = False
    ...
    def get_density(self, pts: torch.Tensor, timestamps: Optional[torch.Tensor] = None):
        ...
        return density, features, grid_features
    ...
    def forward(self,
        ...
        if self.use_appearance_embedding:
            ...
            timestamps = None
        density, features, grid_features = self.get_density(pts, timestamps)
        n_rays, n_samples = pts.shape[:2]

        # color_features_origin = features.clone()
        ###### new ##########
        features = grid_features

        encoded_directions_out = self.direction_encoder(directions)
        ...
        if self.linear_decoder:
            ...
            color_features = [encoded_directions, features.view(-1, self.feature_dim)]
        ...
        if self.linear_decoder:
            ...
            color_features_origin = self.direction_net(color_features)
            rgb = self.color_net(color_features_origin).to(directions).view(n_rays, n_samples, 3)

        ###########################################################

        return {"rgb": rgb, "density": density, "features":color_features_origin, "encoded_direction" : encoded_directions_out}
