...
from .base_trainer_style import BaseTrainer, init_dloader_random, initialize_model
...
class StaticTrainer(BaseTrainer):
    ...
    def eval_step_time_check(self, data, **kwargs) -> MutableMapping[str, torch.Tensor]:
        """
        Note that here `data` contains a whole image. we need to split it up before tracing
        for memory constraints.
        """
...
def init_tr_data(data_downsample: float, data_dirs: Sequence[str], render_only: bool, **kwargs):
    ...
    if dset_type == "synthetic":
        ...
        dset = LLFFDataset(
            ...
            ndc_far=float(kwargs['ndc_far']), near_scaling=float(kwargs['near_scaling']), render_only=render_only)
    else:
...
def init_ts_data(data_downsample, data_dirs: Sequence[str], split: str, **kwargs):
    assert len(data_dirs) == 1
    data_dir = data_dirs[0]
    dset_type = decide_dset_type(data_dir)
    if dset_type == "synthetic":
...
def load_data(data_downsample, data_dirs, validate_only, render_only, **kwargs):
    ...
    if not validate_only:
        od.update(init_tr_data(data_downsample, data_dirs, render_only, **kwargs))
    else:
        od.update(tr_loader=None, tr_dset=None)
    test_split = 'render' if render_only else 'test'
    od.update(init_ts_data(data_downsample, data_dirs, split=test_split,  **kwargs))
    return od
