...
class BaseTrainer(abc.ABC):
    ...
    def train_step(self, data, **kwargs) -> bool:
        ...
        with torch.cuda.amp.autocast(enabled=self.train_fp16):
            ...
            color_feature = fwd_out['color_feature']
            vgg_gt = data['vgg_features'].cuda()

            semantic_loss = self.L1_loss(fwd_out['semantic_feature'], data['semantic_features']) 
            loss += semantic_loss * 0.1
            
            vgg_loss = self.criterion(color_feature, vgg_gt)
            loss += vgg_loss * 0.01
            loss += recon_loss
        ...
        if self.global_step % self.calc_metrics_every == 0:
            with torch.no_grad():
                ...
                self.loss_info[f"semantic_feature"].update(semantic_loss_val)
                vgg_loss_val = vgg_loss.item()
                self.loss_info[f"vgg_loss"].update(vgg_loss_val)
                for r in self.regularizers:
    ...
    def _move_data_to_device(self, data):
        ...
        data["semantic_features"] = data["semantic_features"].to(self.device)
