...
def get_style_EDE(styl_feat, weight=1,device='cuda:0'):
    # styl_feat = get_squeeze_feat(styl_feat)
    s_mean = torch.mean(styl_feat, 1)
    
    styl_feat -= s_mean.unsqueeze(1).expand_as(styl_feat)
    _, s_e, s_v = svd(styl_feat,  device=device)
...
def get_content_EDE(content_feat, weight=1,device='cuda:0'):
    # styl_feat = get_squeeze_feat(styl_feat)
    c_mean = torch.mean(content_feat, 1)
    
    content_feat -= c_mean.unsqueeze(1).expand_as(content_feat)
    _, c_e, c_v = svd(content_feat,  device=device)
    # _, c_e, c_v = svd(content_feat, iden=True, device=device)
    k_c = get_rank(c_e, content_feat.size()[0])
    c_d = (c_e[0:k_c]).pow(-0.5)
    EDE = torch.mm(torch.mm(c_v[:, 0:k_c], torch.diag(c_d) * weight), (c_v[:, 0:k_c].t()))

    return EDE, c_mean

def wct_with_EDE(cont_feat, EDE, s_mean, origin_content_feat, weight=1, registers=None, device='cuda:0'):
    whiten_cF = cont_feat.permute(1,0)

    targetFeature = torch.mm(EDE, whiten_cF)
    targetFeature = targetFeature + s_mean.unsqueeze(1).expand_as(targetFeature)
    cont_min = origin_content_feat.min()
    cont_max = origin_content_feat.max()
    # targetFeature.clamp_(cont_min, cont_max)

    return targetFeature

def wct_with_content_EDE(cont_feat, content_EDE, c_mean, style_EDE, s_mean, weight=1, registers=None, device='cuda:0'):
    cont_feat = cont_feat.permute(1,0) - c_mean.unsqueeze(1).expand_as(cont_feat.permute(1,0))
    whiten_cF = torch.mm(content_EDE, cont_feat)
    
    targetFeature = torch.mm(style_EDE, whiten_cF)
    targetFeature = targetFeature + s_mean.unsqueeze(1).expand_as(targetFeature)
    cont_min = cont_feat.min()
    cont_max = cont_feat.max()
    # targetFeature.clamp_(cont_min, cont_max)
...
def whitening_with_content_EDE(cont_feat, content_EDE, c_mean, style_EDE, s_mean, weight=1, registers=None, device='cuda:0'):
    cont_feat = cont_feat.permute(1,0) - c_mean.unsqueeze(1).expand_as(cont_feat.permute(1,0))
    whiten_cF = torch.mm(content_EDE, cont_feat)
