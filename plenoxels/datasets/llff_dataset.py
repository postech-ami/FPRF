...
from plenoxels.models.dino.save_dino_feature import generate_semantic_feature_maps
from plenoxels.models.vgg.encode_to_vgg import generate_vgg_feature_maps
...
class LLFFDataset(BaseDataset):
    def __init__(self,
        ...
        if split == 'render':
            ...
            image_paths, poses, near_fars, intrinsics, feature_paths, vgg_feature_paths = load_llff_poses(
                datadir, downsample=downsample, split='test', hold_every=1,
                near_scaling=self.near_scaling, render_only = render_only)
            render_poses = generate_spiral_path(
                poses.numpy(), near_fars.numpy(), n_frames=120, n_rots=2, zrate=0.5,
                dt=self.near_scaling, percentile=70)
            self.poses = torch.from_numpy(render_poses).float()

        else:
            image_paths, self.poses, near_fars, intrinsics, feature_paths, vgg_feature_paths = load_llff_poses(
            ...
            # semantic_features = torch.zeros(len(image_paths),intrinsics.height, intrinsics.width, 384, dtype=torch.half)
            if not render_only and split == 'train':
                semantic_features = load_pt_files(feature_paths, intrinsics, split, 384)
                semantic_features = semantic_features.reshape(-1, semantic_features.shape[-1])
            
            # vgg_features = torch.zeros(len(image_paths),intrinsics.height, intrinsics.width, 128, dtype=torch.half)
            if not render_only and split == 'train':
                vgg_features = load_pt_files(vgg_feature_paths, intrinsics, split, 128)
                vgg_features = vgg_features.reshape(-1, vgg_features.shape[-1])
    ...
    def __getitem__(self, index):
        ...
        if self.imgs is not None:
            ...
            out["semantic_features"] = self.semantic_features[index]
            out["vgg_features"] = self.vgg_features[index]
        else:
            out["imgs"] = None
...
def load_llff_poses(datadir: str,
                    ...
                    split: str,
                    hold_every: int,
                    near_scaling: float = 0.75, render_only = False) -> Tuple[
    ...
    if render_only or split != "train":
        semantic_feature_paths = None
        vgg_feature_paths = None
    else:
        if not os.path.exists(os.path.join(datadir, f'dino_features_{int_dsample}/')) and split == "train":
            generate_semantic_feature_maps(datadir, int_dsample)
        semantic_feature_paths = sorted(glob.glob(os.path.join(datadir, f'dino_features_{int_dsample}/*')))
        semantic_feature_paths = [semantic_feature_paths[i] for i in img_list]

        if not os.path.exists(os.path.join(datadir, f'vgg_features_{int_dsample}/')) and split == "train":
            generate_vgg_feature_maps(datadir, int_dsample)
        vgg_feature_paths = sorted(glob.glob(os.path.join(datadir, f'vgg_features_{int_dsample}/*')))
        vgg_feature_paths = [vgg_feature_paths[i] for i in img_list]

    return image_paths, poses, near_fars, intrinsics, semantic_feature_paths, vgg_feature_paths
...
def load_pt_files(image_paths: List[str], intrinsics: Intrinsics, split: str, feat_dim):
    all_rgbs = torch.zeros(len(image_paths),feat_dim,intrinsics.height, intrinsics.width, dtype=torch.half)
    for i in tqdm(range(len(image_paths)), desc="Loading image features"):
        all_rgbs[i] = torch.load(image_paths[i])[:,:,:intrinsics.height, :intrinsics.width]
    all_rgbs = all_rgbs.permute(0,2,3,1)
    return all_rgbs
