...
from plenoxels.runners import static_trainer_style
...
def load_data(model_type: str, data_downsample, data_dirs, validate_only: bool, render_only: bool, **kwargs):
    ...
    return static_trainer_style.load_data(
...
def init_trainer(model_type: str, **kwargs):
    from plenoxels.runners import static_trainer_style
    return static_trainer_style.StaticTrainer(**kwargs)
...
def main():
    ...
    p.add_argument('--style_path', type=str, default="references")
    p.add_argument('--num_clusters', type=int, default=10)
    p.add_argument('--local_global_blending_ratio', type=float, default=0.3)
    p.add_argument('--temperature', type=float, default=100)
    ...
    config['style_path'] = args.style_path
    config['num_clusters'] = args.num_clusters
    config['local_global_blending_ratio'] = args.local_global_blending_ratio
